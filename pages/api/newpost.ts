...
handler.post(async (req: NextApiRequest, res: NextApiResponse)=>{
    ...
    if (validBody && mongoConnect) {
        let insertCursor = await mongoClient.db('Portfolio').collection('Posts').insertOne({
        ...
        if (insertCursor) {
          insertCursor.id = insertCursor.local_middleware
          delete insertCursor._id
          res.send({status: 'success', post: insertCursor, log: 'Post inserted'})
        }
